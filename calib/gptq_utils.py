"""GPTQ baseline weight quantization (OBS-style rounding with the input Hessian)."""
...
def gptq_fwrd(model, dataloader, dev, args):
    """GPTQ quantization pipeline (adapted from the GPTQ repo)."""
    ...
    class Catcher(nn.Module):
        ...
        def forward(self, inp, *args, **kwargs):
            ...
            cache['layer_kwargs'] = kwargs  # all kwargs for Gemma3 dual-RoPE etc.
            raise ValueError
    ...
    def layer_forward(layer, inp):
        return layer(inp.unsqueeze(0), **cache['layer_kwargs'])[0]
        ...
        for names in sequential:
            ...
            for name in subset:
                ...
                if args.int8_down_proj and 'down_proj' in name:
                    layer_weight_bits = 8
                # Inter-layer MP: per-sublayer bit override
                if getattr(args, 'inter_layer_mp', None):
                    from inter_layer_mp import get_sublayer_bits
                    full_name = f'layers.{i}.{name}'
                    layer_weight_bits = get_sublayer_bits(full_name, args.w_bits, args.inter_layer_mp)
...
def rtn_fwrd(model, dev, args):
    """Round-to-nearest baseline (adapted from the GPTQ repo)."""
