"""Weight/activation quantizers and the ActQuantWrapper around nn.Linear."""
...
class ActQuantWrapper(torch.nn.Module):
    '''
        Wraps an nn.Linear: applies an optional online Hadamard transform to the input,
        quantizes input/output activations, and calls the wrapped module in between.
    '''
    ...
    def __init__(self, module: torch.nn.Linear):
        ...
        # Placeholder buffer (set to None) so load_state_dict can restore had_K with arbitrary shape.
    ...
    def extra_repr(self) -> str:
        ...
        if self.quantizer.bits < 16:
            str_ += ' (Asymmetric Per-Token)' if not self.quantizer.sym else ' (Symmetric Per-Token)'

        str_ += f'\nOutput Quantizer Bits: {self.out_quantizer.bits}'
        if self.out_quantizer.bits < 16:
            str_ += ' (Asymmetric Per-Token)' if not self.out_quantizer.sym else ' (Symmetric Per-Token)'
